...
public class AnalyticDaoJDBC implements AnalyticDAO {
	private DBConnection dbConnection;

	public AnalyticDaoJDBC(DBConnection dbConnection) {
		this.dbConnection = dbConnection;
	}

	public void save(Analytic analytic) {
		
		if ( (analytic.getPagina() == null) ){
			throw new PersistenceException("Analytic non memorizzato: un utente deve avere almeno un numero di telefono e una mail");
		}
		Connection connection = this.dbConnection.getConnection();
		try {
			
			String insert = "insert into analytics(Pagina, DataOra, NumeroTelefono, idLocale) values (?,?,?,?)";
			PreparedStatement statement = connection.prepareStatement(insert);
			statement.setString(1, analytic.getPagina());
			
            SimpleDateFormat datetime = new SimpleDateFormat("yyyy-MM-dd HH:mm");            
			statement.setString(2, datetime.format(analytic.getDataOra()));
			
			statement.setString(3, analytic.getNumeroTelefono());
			statement.setLong(4, analytic.getIdLocale());
...
//				}
	...
	public List<Analytic> findAllByLocal(Long idLocale) {
		...
		try {			
			Analytic analytic;
			PreparedStatement statement;
			String query = "select * from analytics where idLocale=?";
			statement = connection.prepareStatement(query);
			...
			while (result.next()) {
				...
				analytic.setDataOra(result.getDate("DataOra"));
